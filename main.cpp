...
int main() {
    ...
    // Basic arithmetic
    sv_int4 int_sum = sv_int_add(int_a, int_b);
    sv_int4 int_diff = sv_int_sub(int_b, int_a);
    sv_int4 int_prod = sv_int_mul(int_a, int_b);
    ...
    // Masked operations
    sv_int4 masked_add = sv_int_masked_add(int_a, int_ones, int_lt_mask);
    ...
    // Basic arithmetic
    sv_float4 float_sum = sv_float_add(float_a, float_b);
    sv_float4 float_prod = sv_float_mul(float_a, float_b);
    sv_float4 float_sqrt_a = sv_float_sqrt(float_a);
    ...
    // Advanced operations
    sv_float4 float_min = sv_float_min(float_a, float_b);
    sv_float4 float_max = sv_float_max(float_a, float_b);
    ...
    bool all_true = sv_mask_all(mask1);
    bool any_true = sv_mask_any(mask2);
    
    std::cout << "All true in mask1: " << (all_true ? "Yes" : "No") << std::endl;
    std::cout << "Any true in mask2: " << (any_true ? "Yes" : "No") << std::endl;
    
    // More masked operations
    sv_float4 masked_float_mul = sv_float_masked_mul(float_a, float_b, mask2);
